...
{
    public class DBAccessFhirSingleton : IDBAccess
    {
        ...
        public DBAccessFhirSingleton(IConfiguration configuration, IOptions<DatabaseSettings> dbSettings)
        {
        ...
        public async Task<int> Execute(string sp, DynamicParameters parms, string ConnectionString, CommandType commandType = CommandType.StoredProcedure)
        {
            ...
            return await db.ExecuteAsync($"{_schema}.{sp}", parms, commandType: commandType, commandTimeout: 180);
        }
        ...
        {
            using IDbConnection db = new SqlConnection(ConnectionString);
            return (await db.QueryAsync<T>($"{_schema}.{sp}", parms, commandType: commandType, commandTimeout: 180)).FirstOrDefault()!;
        }
        ...
        {
            using IDbConnection db = new SqlConnection(ConnectionString);
            return (await db.QueryAsync<T>($"{_schema}.{sp}", parms, commandType: commandType, commandTimeout: 180)).ToList();
        }
        ...
        public async Task<T> Insert<T>(string sp, DynamicParameters parms, string ConnectionString, CommandType commandType = CommandType.StoredProcedure)
        {
            ...
            try
            {
                ...
                try
                {
                    result = (await db.QueryAsync<T>($"{_schema}.{sp}", parms, commandType: commandType, commandTimeout: 180)).FirstOrDefault()!;
                }
        ...
        {
            using IDbConnection db = new SqlConnection(ConnectionString);
            var reader = await db.QueryMultipleAsync($"{_schema}.{sp}", parms, commandType: commandType, commandTimeout: 180);
